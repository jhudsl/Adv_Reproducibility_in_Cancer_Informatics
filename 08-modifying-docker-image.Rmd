...
When creating your new DockerHub repository, the name you give it must match the image name you will use in the `docker build` command — specifically the part **after** your username. Docker image names used with DockerHub follow the format `your_dockerhub_username/image_name`. For example, if you are following along with the R example, you might name your repository `reproducible-r`, and later build your image as `your_dockerhub_username/reproducible-r`. You can put a description and click create. On the right it shows how you can interact with this from your local command line.
...
Before pushing, make sure you are logged in to DockerHub from the command line by running:

```
docker login
```

You will be prompted for your DockerHub username and password. No extra flags are needed.

Then go to your [local command line](https://towardsdatascience.com/a-quick-guide-to-using-command-line-terminal-96815b97b955) and use the push command specified on the right side of your repository page. It will look like:

```
docker push your_dockerhub_username/image_name
```

You don't have to specify a tagname if you don't want to. If you don't want to specify a tagname, leave off the `:tagname` if you like.
...
Build a docker image from this Dockerfile using the command below. Replace `your_dockerhub_username` with your actual DockerHub username, and replace `image_name` with what you would like your image to be called (e.g. `reproducible-r` or `reproducible-python`). **Including your username in the image name is required for `docker push` to work later.**

```
docker build -f Dockerfile . -t your_dockerhub_username/image_name
```

For example, if your DockerHub username is `janesmith` and you are following the R example:
```
docker build -f Dockerfile . -t janesmith/reproducible-r
```

> **Note for Apple Silicon users (M1/M2/M3 Macs):** You may see an error like `no match for platform in manifest: not found`. Add the `--platform=linux/amd64` flag to your build command:
> ```
> docker build --platform=linux/amd64 -f Dockerfile . -t your_dockerhub_username/image_name
> ```
> Docker Desktop may show a warning about running under emulation — this is expected and the image will still work for course purposes. You will need to add this flag to all subsequent `docker build` commands as well.
...
Now re-run `docker build` as you did in the previous section. (Use the command below but replace `your_dockerhub_username/image_name` with your username and image name as before).
...
=> => naming to docker.io/your_dockerhub_username/image_name
```
...
But replace `your_dockerhub_username/image_name` with your username and image name.
```
docker run --rm -v $PWD:/home/jovyan/work -e JUPYTER_ENABLE_LAB=yes -p 8787:8787 your_dockerhub_username/image_name
```
</details>
...
But replace `your_dockerhub_username/image_name` with your username and image name.
```
docker run --rm -v $PWD:/home/rstudio -e PASSWORD=password -p 8787:8787 your_dockerhub_username/image_name
```
</details>
